...
var TileSpriteWebGLRenderer = function (renderer, src, interpolationPercentage, camera, parentMatrix) {
    ...
    this.pipeline.batchTexture(src, src.tileTexture, src.frame.width * src.tileScaleX, src.frame.height * src.tileScaleY, src.x, src.y, src.width, src.height, src.scaleX, src.scaleY, src.rotation, src.flipX, src.flipY, src.scrollFactorX, src.scrollFactorY, src.originX * src.width, src.originY * src.height, 0, 0, src.width, src.height, getTint(src._tintTL, camera.alpha * src._alphaTL), getTint(src._tintTR, camera.alpha * src._alphaTR), getTint(src._tintBL, camera.alpha * src._alphaBL), getTint(src._tintBR, camera.alpha * src._alphaBR), (src._isTinted && src.tintFill), (src.tilePositionX % src.frame.width) / src.frame.width, (src.tilePositionY % src.frame.height) / src.frame.height, camera, parentMatrix);
};
module.exports = TileSpriteWebGLRenderer;
//# sourceMappingURL=TileSpriteWebGLRenderer.js.map