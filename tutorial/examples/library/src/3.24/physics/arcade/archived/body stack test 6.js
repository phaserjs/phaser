...
function create ()
{
    ...
    player = createBody({ x: 400, y: 260 });

    createBody({ x: 360, y: 400 }).setVelocityY(0);
    createBody({ x: 440, y: 400 }).setVelocityY(0);

    createBody({ x: 360, y: 100 }).setVelocityY(0);
    createBody({ x: 440, y: 100 }).setVelocityY(0);
...
function update (time)
{
    ...
    monitor.setVelocity(0);

    if (cursors.left.isDown)
    {
        monitor.setVelocityX(-200);
    }
    ...
    {
        monitor.setVelocityY(-200);
        // monitor.body.velocity.y = -200;
    }
