...
function preload ()
{
    this.load.atlas('megaset', 'assets/atlas/megaset-3.png', 'assets/atlas/megaset-3.json');
}
...
function create ()
{
    ...
    quad = this.add.quad(400, 300, 'megaset', 'exocet_spaceman');

    var text = this.add.text(10, 10).setColor('#ffffff').setText('Click me to change frame').setInteractive();

    text.on('pointerup', function () {

        var key = quad.frame.key;

        do
        {
            var newFrame = Phaser.Utils.Array.GetRandom(frames);
        } while (newFrame.key !== key);

        quad.setFrame(newFrame);
