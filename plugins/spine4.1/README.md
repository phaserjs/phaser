...
2. `cd spine-runtimes` and then run `git checkout 4.1`
~~3. Run `npm i` inside the `spine-runtimes/spine-ts` folder.~~
4. From the root directory, run `npm run plugin.spine4.1.runtimes` to build the new runtimes to the `plugins/spine4.1/src/runtimes` folder.
